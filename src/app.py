...
df = pd.read_csv(r"https://github.com/Ewurama-A/Unidatarender-app/blob/main/merged_Uniersity_data.csv?raw=true")
...
app = Dash(__name__, external_scripts=external_script, meta_tags=[{'name':'viewport','content':'width=device-width,initial-scale=1.0,maximum-scale=1.2,minimum-scale=0.5,'}])
server = app.server
...
app.layout = html.Div([
  html.H1("Unidata: Universities of the world", id ='home', className=" shadow-lg w-auto text-white text-4xl italic p-6 m-2 rounded-md text-center bg-[#3b0764] shadow-lg"),
   ...
   html.Div([
        html.A( href ='#home', id = 'home', children = 'Home', className = 'shadow-md transition-colors duration-500 ease-in-out hover:bg-[#cbd5e1] hover:opacity-75 transform hover:-translate-y-1 hover:scale-100 rounded-sm text-white p-4 font-semibold cursor-pointer block italic bg-[#312e81] shadow-md'),
        html.A( href ='#graph', id = 'graph', children = 'Graphs', className = 'shadow-md transition-colors duration-500  ease-in-out hover:bg-[#cbd5e1] underline hover:opacity-75 transform hover:-translate-y-1 hover:scale-100 rounded-sm text-white p-4 font-semibold cursor-pointer block italic bg-[#312e81] shadow-md'),
        html.A( href ='#info', id = 'info', children = 'University Information', className = 'shadow-md transition-colors  duration-500 ease-in-out hover:bg-[#cbd5e1] hover:opacity-75 transform hover:-translate-y-1 hover:scale-100 rounded-sm text-white p-4 font-semibold cursor-pointer block italic bg-[#312e81] shadow-md')],
        className = 'shadow-md flex flex-col w-auto p-2 m-2 mt-1 bg-[#312e81] divide-y-4 divide-white justify evenly opacity-75 space-x-4 mb-2 rounded-md md:flex-row md:divide-x-4 md:justify-start shadow-md'),
  ...
  html.Div([  
    ...
    html.Div([
      html.Div([
        ...
        html.Div(children=[
             html.Span(uni_num, className="text-5xl text-center my-2 font-bold md:pt-0"),
             html.Span("Universities available", className="text-lg fontmedium text-center ml-1"),
         ],className=" shadow-lg flex flex-col justify-center p-2 text-white itemscenter rounded-md bg-[#4c1d95] md:px-2 md:mx-4 shadow-lg"),
         
           html.Div(children=[
             html.Span(location_num, className="text-5xl text-center my-2 font-bold"),
             html.Span("Total locations available", className="text-lg fontmedium ml-1 text-center"),
         ...
         ],className=" shadow-lg flex flex-col justify-center p-4 text-white itemscenter rounded-md bg-[#4c1d95] shadow-lg md:p-4")],className="flex flex-col justify-center rounded-md w-auto p-4 m-4 space-y-8 bg-[#e4e4e7] md:flex-row md:justify-center md:space-x-12 md:p-4 md:m-4",id = 'home'),
     
       html.Div([
         html.Div(dcc.Graph(id='locationgraph',figure =location_graph),className="shadow-lg  rounded-md row-span-2 shadow-lg"),
         ...
         html.Div([
           ...
           ],id= 'graph', className='grid grid-cols-1 grid-rows-4 md:grid-cols-2 md:grid-rows-4 rounded-md gap-8 md:gap-4 m-2 px-4 py-8 bg-[#e4e4e7]')
        
    ],className = 'md:row-span-3 w-auto'),
        
   ],className = 'grid md:grid-rows-3 gap-2 w-full'),
    
   html.Div(children = "Ama Assiamah. Data Copyright: Kaggle. @2023", id= 'info', className =" w-auto text-white text-md italic p-4 text-center bg-[#4c1d95]")],
   className="flex flex-col bg-[#e4e4e7] max-w-full xs:min-w-0 ")
